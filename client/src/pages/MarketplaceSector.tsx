...
export default function MarketplaceSector() {
  ...
  const liveSector = (sectors.data ?? []).find((item) => item.slug === slug);
  const catalogSector = slug ? sectorCatalog.find((item) => item.slug === slug) : undefined;
  const sector = liveSector ?? (catalogSector && slug ? { id: undefined, slug, labelAr: catalogSector.labelAr, labelEn: catalogSector.labelEn, labelFr: catalogSector.labelFr, descriptionAr: catalogSector.descriptionAr, descriptionEn: catalogSector.descriptionAr, descriptionFr: catalogSector.descriptionAr, icon: catalogSector.icon, color: catalogSector.color, listingCount: 0 } : undefined);
        ...
        <>
          <section className="relative isolate overflow-hidden px-5 pb-10 pt-12 md:px-8 md:pt-16">
            ...
            <div className="relative mx-auto max-w-7xl">
              ...
              {catalogSector && <div className="mt-5 flex flex-wrap gap-2">{catalogSector.categories.map((category) => <span key={category.ar} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-300">{language === "ar" ? category.ar : language === "fr" ? category.fr : category.en}</span>)}</div>}
            </div>
          </section>
